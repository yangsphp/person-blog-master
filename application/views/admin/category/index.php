...
<section class="content">
    ...
    <div class="row">
        <div class="col-xs-12">
            <div class="box box-solid">
                <div class="box-body">
                    <table style="" id="categoryTable" class="table table-hover radius" cellspacing="0" width="100%">
                        <thead>
                        <tr>
                            <th>ID</th>
                            <th>栏目名称</th>
...
<script>
    ...
    $(document).ready(function () {
        ...
        initTable = $("#categoryTable").dataTable($.dataTablesSettings);
    ...
    function showModal(id=0) {
        showLoading();
        $.get(siteUrl + "/category/add?id="+id, function (data) {
            hideLoading();
            var add_category = layer.open({
                type: 1,
                title: data.title,
                area: '530px',
                closeBtn: 2,
            ...
            $("#close-modal").on("click", function () {
                closeLayer(add_category);
            });
            $("#submit-form").on("click", function () {
                doSubmit(id, add_category)
            });
        }, 'json');
    }
    function doSubmit(id, add_category) {
        var obj = $("#form"), action = 'add_op';
        loadT = layer.msg('正在提交数据...', { time: 0, icon: 16, shade: [0.3, '#000'] });
        if (id > 0) {
            action = 'edit_op';
        }
        $.post(siteUrl+"/category/"+action, obj.serialize(), function (res) {
            if (res.code == 0) {
    ...
    function del(id) {
        var delete_category = layer.open({
            type: 1,
            title: "信息",
            area: '300px',
            closeBtn: 2,
            ...
            success() {
                $("#close-modal").on("click", function () {
                    closeLayer(delete_category);
                });
                $("#submit-form").on("click", function () {
                    doDelete(id, delete_category)
                });
            }
        });
    }
    function doDelete(id, delete_category) {
        loadT = layer.msg('正在提交数据...', { time: 0, icon: 16, shade: [0.3, '#000'] });
        $.get(siteUrl+"/category/delete_op?id="+id, function (res) {
            if (res.code == 0) {
                layer.msg(res.msg, {icon: 1});
                closeLayer(delete_category);
                initTable.api().draw(false);
    ...
    function setStatus(id, flag) {
        loading();
        $.get(siteUrl+"/category/setStatus?id="+id+"&status="+flag, function (res) {
            if (res.code == 0) {
                layer.msg(res.msg, {icon: 1});
                initTable.api().draw(false);
            } else{
