...
<section class="content">
    ...
    <div class="row">
        <div class="col-xs-12">
            <div class="box box-solid">
                <div class="box-body">
                    <table style="" id="tagsTable" class="table table-hover radius" cellspacing="0" width="100%">
                        <thead>
                        <tr>
                            <th>ID</th>
                            <th>标签名称</th>
...
<script>
    ...
    $(document).ready(function () {
        ...
        initTable = $("#tagsTable").dataTable($.dataTablesSettings);
    ...
    function showModal(id=0) {
        showLoading();
        $.get(siteUrl + "/tags/add?id="+id, function (data) {
            hideLoading();
            ...
            $("#submit-form").on("click", function () {
                doSubmit(id, add_tags)
            });
        }, 'json');
    }
    function doSubmit(id, add_tags) {
        var obj = $("#form"), action = 'add_op';
        loadT = layer.msg('正在提交数据...', { time: 0, icon: 16, shade: [0.3, '#000'] });
        if (id > 0) {
            action = 'edit_op';
        }
        $.post(siteUrl+"/tags/"+action, obj.serialize(), function (res) {
            if (res.code == 0) {
    ...
    function del(id) {
        var delete_tags = layer.open({
            ...
            success() {
                $("#close-modal").on("click", function () {
                    closeLayer(delete_tags);
                });
                $("#submit-form").on("click", function () {
                    doDelete(id, delete_tags)
                });
            }
        });
    }
    function doDelete(id, delete_tags) {
        loadT = layer.msg('正在提交数据...', { time: 0, icon: 16, shade: [0.3, '#000'] });
        $.get(siteUrl+"/tags/delete_op?id="+id, function (res) {
            if (res.code == 0) {
                layer.msg(res.msg, {icon: 1});
                closeLayer(delete_tags);
                initTable.api().draw(false);
    ...
    function setStatus(id, flag) {
        loading();
        $.get(siteUrl+"/tags/setStatus?id="+id+"&status="+flag, function (res) {
            if (res.code == 0) {
                layer.msg(res.msg, {icon: 1});
                initTable.api().draw(false);
            } else{
